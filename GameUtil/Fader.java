...
/**
 * Simple fader class. instead of doing painfully slow pixel wise gamma manipulation, use a square
 * with transparency with is drawn over the whole graphic context.
 *
 * @author Volker Oth
 */
public class Fader {
  ...
  public static synchronized void fade(final Graphics g, final int width, final int height) {
    switch (fadeState) {
      case IN:
        ...
        break;
      case OUT:
        if (fadeValue <= MAX_ALPHA - fadeStep) fadeValue += fadeStep;
        else {
          fadeValue = MAX_ALPHA;
          fadeState = State.OFF;
        }
