 ...
 * - if a key is pressed and released, a single SINGLE_CLICK is returned - if a key is pressed for
 * longer than timeFirstPress but not released, a SINGLE_CLICK event is returned. Then if the key is
 * still not released, each timeRepeat microseconds, a new SINGLE_CLICK is returned. - if a key is
 * pressed, released and pressed again within timeDoubleClick microseconds, a DOUBLE_CLICK is fired.
 *
 * @author Volker Oth
 */
public class KeyRepeat {
   ...
   * Constructor.
   *
   * @param tfirst time after which a pressed (and not released) key is recognized as key press
   *     (microseconds)
