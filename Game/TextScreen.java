...
public class TextScreen {
  ...
  /** Draw the intro dialog. */
  static void drawIntro(final int width, final boolean force) {
    synchronized (monitor) {
      if (force || mode != Mode.INTRO) {
        mode = Mode.INTRO;
      ...
      if (++rotCtr > maxRotCtr) {
        ...
        textDialog.drawImage(
            imgTrg, -120 - (int) (imgSrc.getHeight() / 2 * Math.abs(rotFact) + 0.5));
      } else {
  ...
  static void drawBriefing(final int width, final boolean force) {
    synchronized (monitor) {
      if (force || mode != Mode.BRIEFING) {
        mode = Mode.BRIEFING;
        textDialog.init(width);
  ...
  static void drawDebriefing(final int width, final boolean force) {
    synchronized (monitor) {
      if (force || mode != Mode.DEBRIEFING) {
        mode = Mode.DEBRIEFING;
        textDialog.init(width);
        ...
        if (GameController.wasLost()) {
          ...
          if (lp.getLevelCount(GameController.getCurDiffLevel()) > ln + 1) {
            ...
            String code =
                LevelCode.create(lp.getCodeSeed(), absLevel, rescued, 0, lp.getCodeOffset());
        ...
        textDialog.addTextButton(
            -4, 5, BUTTON_SAVEREPLAY, "Save Replay", "Save Replay", BLUE, BROWN);
      }
