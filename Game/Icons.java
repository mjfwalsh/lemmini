...
public class Icons {
  ...
  public static Type getType(final int x) {
    if (x >= (LAST_DRAWN + 1) * WIDTH) return Type.INVALID; // invalid
  ...
  static boolean isPressed(Type type) {
    ...
    if (idx > LAST_DRAWN) return false;
  ...
  static void press(final Type type) {
    ...
    switch (type) {
      ...
      case FFWD:
        ...
        if (idx <= LAST_DRAWN) iconGfx.drawImage(icons[idx].getImage(), WIDTH * idx, 0, null);
        break;
      case CLIMB:
      case FLOAT:
      case BOMB:
