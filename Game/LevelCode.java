...
/**
 * @author Volker Oth Create and evaluate lemmings level codes Based on the documentation and Basic
 *     code of Herman Perk (LemGen)
 */
public class LevelCode {
  ...
  public static String create(
      final String seed, int lvl, final int percent, final int magic, final int offset) {
  ...
  public static int getLevel(final String seed, final String code, final int offset) {
    ...
    if (seed.length() != 10 || code.length() != 10) return -1;

    int level = ((bi[7] - bs[7]) & 0xf) + (((bi[8] - bs[8]) & 0xf) << 4);
    // unrotate
    for (int j = 0; j < 7; j++) bo[(j + 6 + (level % 8)) % 7] = bi[j];
    // decode
    int level_ = 0;
    int percent = 0;
    for (int i = 0; i < 7; i++) {
    ...
    if (level != level_ || percent > 100) return -1;

    level -= offset;
    while (level < 0) level += MAX_LVL_NUM;
