...
public class Level {
  ...
  public static final int DEFAULT_PARTICLE_COLORS[] = {
    0xff00ff00, 0xff0000ff, 0xffffffff, 0xffffffff, 0xffff0000
  };

  /** array of default styles */
  private static final String STYLES[] = {
    "dirt", "fire", "marble", "pillar", "crystal", "brick", "rock", "snow", "Bubble", "special"
  };
  ...
  void loadLevel(final String fname) throws ResourceException, LemmException {
    ...
    Props p = new Props();
    if (!p.load(fname)) throw new ResourceException(fname);
  ...
  Stencil paintLevel(final BufferedImage bgImage, final Stencil s) {
    ...
    if (s != null && s.getWidth() == bgWidth && s.getHeight() == bgImage.getHeight()) {
      s.clear();
      stencil = s;
    } else stencil = new Stencil(bgWidth, bgImage.getHeight());
    // paint terrain
    for (int n = 0; n < terrain.size(); n++) {
      ...
      PixelGrabber pixelgrabber = new PixelGrabber(i, 0, 0, width, height, source, 0, width);
      try {
      ...
      try {
        for (int y = 0; y < height; y++) {
          if (y + ty < 0 || y + ty >= bgHeight) continue;
          int yLineStencil = (y + ty) * bgWidth;
          int yLine;
          if (upsideDown) yLine = (height - y - 1) * width;
          else yLine = y * width;
          for (int x = 0; x < width; x++) {
            if (x + tx < 0 || x + tx >= bgWidth) continue;
            int col = source[yLine + x];
            // ignore transparent pixels
            if ((col & 0xff000000) == 0) continue;
            boolean paint = false;
            if (!overwrite) {
        ...
        if (!o.upsideDown) {
          for (int y = 0; y < spr.getHeight(); y++) {
            ...
            for (int x = 0; x < spr.getWidth(); x++) {
              ...
              if (
              /*paint &&*/ spr.getType() != SpriteObject.Type.ENTRY
            ...
            if (inFront) {
              for (int y = 0; y < spr.getHeight(); y++) {
                ...
                for (int x = 0; x < spr.getWidth(); x++) {
                  ...
                  boolean paint =
                      drawFull
                          || (stencilValMasked != 0 && drawOnVis)
                          || (stencilValMasked == 0 && noOverwrite);
                  // hack for overlap:
                  int id = Stencil.getObjectID(stencilVal);
                  // check if a different interactive object was already entered at this pixel
                  // position
                  ...
                  if ((imgCol & 0xff000000) == 0) continue;
                  if (!paint) imgSpr.setRGB(x, y, imgCol & 0xffffff); // set transparent
                }
              }
            }
    ...
    for (int n = 0; n < steel.size(); n++) {
      ...
      for (int y = 0; y < stl.height; y++) {
        if (y + sy < 0 || y + sy >= bgHeight) continue;
        int yLineStencil = (y + sy) * bgWidth;
        for (int x = 0; x < stl.width; x++) {
          if (x + sx < 0 || x + sx >= bgWidth) continue;
  ...
  private SpriteObject[] loadObjects(final String set) throws ResourceException {
    ...
    for (int i = 0; i < particleCol.length; i++) particleCol[i] |= 0xff000000;
      ...
      for (int y = 0; y < img.getHeight(); y++)
        for (int x = 0; x < img.getWidth(); x++) {
          ...
          if (c == bgCol)
            ...
            int sum = 0;
            for (int i = 0; i < 3; i++, c >>= 8) sum += (c & 0xff);
            sum /= 3; // mean value
            if (sum != 0) sum += 0x60;
            // sum *= 3; // make lighter
...
class LvlObject {
  ...
  /**
   * paint mode: don't overwrite terrain pixel in the current (!) background image. special
   * NO_OVERWRITE case for objects hidden behind terrain.
   */
